...
PIPELINE_REPLACE_INPUT_SLICES="${PIPELINE_REPLACE_INPUT_SLICES:-0}"
PIPELINE_REPLACE_SIMILARITY="${PIPELINE_REPLACE_SIMILARITY:-0}"
SCORE_SNAPSHOT_ENABLED="${SCORE_SNAPSHOT_ENABLED:-1}"
SCORE_SNAPSHOT_SEASONS="${SCORE_SNAPSHOT_SEASONS:-2026/2027,2026}"
SCORE_SNAPSHOT_CADENCE="${SCORE_SNAPSHOT_CADENCE:-biweekly}"
  ...
  if [[ "$PIPELINE_RUNNER" == "docker" ]]; then
    (
      ...
      docker compose --env-file "$DOCKER_ENV_FILE" -f "$DOCKER_COMPOSE_FILE" run --rm \
        ...
        -e SCORE_SNAPSHOT_ENABLED="$SCORE_SNAPSHOT_ENABLED" \
        -e SCORE_SNAPSHOT_SEASONS="$SCORE_SNAPSHOT_SEASONS" \
        -e SCORE_SNAPSHOT_CADENCE="$SCORE_SNAPSHOT_CADENCE" \
        -e SIM_TOPK="$SIM_TOPK" \
        pipeline \
    ...
    (
      ...
      SCORE_SNAPSHOT_ENABLED="$SCORE_SNAPSHOT_ENABLED" \
      SCORE_SNAPSHOT_SEASONS="$SCORE_SNAPSHOT_SEASONS" \
      SCORE_SNAPSHOT_CADENCE="$SCORE_SNAPSHOT_CADENCE" \
      SIM_TOPK="$SIM_TOPK" \
