...
import { clampPercentile, percentileColor, strengthLevel, weaknessLevel } from "@/lib/percentileColors";
import {
  getMetricConfig,
  getPositionMeta,
  getRadarMetricKeys,
...
const Kicker = ({ children }) => (
  <p className="text-[11px] font-black uppercase tracking-[0.22em] text-teal-700">{children}</p>
);
...
const scoreBarTone = (value) => {
  ...
  if (score === null) return { color: "#94a3b8", track: "#f1f5f9" };
  if (score < 40) return { color: "#dc2626", track: "#fee2e2" };
  if (score < 70) return { color: "#d97706", track: "#fef3c7" };
  return { color: "#0f766e", track: "#ccfbf1" };
};
...
export function PlayerSearch({ query, results, visible, onQueryChange, onFocus, onSelect }) {
  return (
    <div className="relative z-[9999]">
      <label htmlFor="report-player-search" className="text-[11px] font-black uppercase tracking-[0.22em] text-teal-700">Player search</label>
      <input
      ...
      {visible && query.trim().length >= 2 ? (
        <div className="absolute z-[9999] mt-2 max-h-80 w-full overflow-auto rounded-lg border border-slate-200 bg-white shadow-2xl">
          {results.length ? results.map((player) => (
            <button
              key={`${player.id}-${player.player_season_id || "latest"}`}
              type="button"
              className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm text-slate-700 transition hover:bg-teal-50"
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => onSelect(player)}
            >
              <span className="min-w-0">
                <span className="block truncate font-black text-slate-950">{player.name}</span>
              ...
              <span className="shrink-0 rounded-full border border-slate-200 px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-teal-700">Select</span>
            </button>
      ...
      {seasons.map((season) => {
        ...
        return (
          <button
            ...
            className={`flex min-w-[210px] items-center gap-3 rounded-lg border px-3 py-3 text-left transition ${active ? "border-teal-600 bg-teal-50 shadow-sm" : "border-slate-200 bg-white hover:border-teal-500"}`}
          >
            <ClubLogo name={season.team} className="h-10 w-10 rounded-xl" />
            <span className="min-w-0">
              <span className={`block truncate text-sm font-black ${active ? "text-teal-900" : "text-slate-900"}`}>{season.team || "Unknown club"}</span>
...
export function SeasonStatistics({ report, metrics }) {
  ...
  return (
    <ReportCard className="border-teal-200 bg-gradient-to-br from-white to-teal-50/70">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <Kicker>Statistics</Kicker>
          <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">Season executive summary</h2>
        </div>
        <div className="rounded-lg border border-teal-200 bg-white px-5 py-3 text-right shadow-sm">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-teal-700">Rating Next Legend</p>
          <p className="text-4xl font-black tabular-nums text-teal-700">{formatValue(player.global_score_adjusted, "score")}</p>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-6">
        {stats.map((stat) => (
          <div key={stat.label} className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-3">
            <p className="truncate text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{stat.label}</p>
            <p className="mt-2 truncate text-lg font-black tabular-nums text-slate-900">{stat.format === "text" ? (stat.value || "-") : formatValue(stat.value, stat.format)}</p>
          </div>
        ))}
      </div>
    </ReportCard>
    ...
    <ReportCard className="min-h-[360px]">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-xl font-black text-slate-950">{meta.label}</h3>
        </div>
        <span className="rounded-full border border-teal-600/50 bg-teal-50 px-3 py-1 text-sm font-black text-teal-700">{meta.short}</span>
      </div>
      <div className="relative mx-auto mt-5 h-[260px] max-w-[210px] overflow-hidden rounded-lg border border-teal-200 bg-gradient-to-b from-teal-50 to-white shadow-inner">
        <div className="absolute inset-3 rounded-lg border border-teal-300/70" />
        <div className="absolute left-1/2 top-1/2 h-20 w-20 -translate-x-1/2 -translate-y-1/2 rounded-full border border-teal-300/70" />
        <div className="absolute left-3 right-3 top-1/2 h-px bg-teal-300/70" />
        <div className="absolute left-1/2 top-3 h-10 w-24 -translate-x-1/2 rounded-b-full border-x border-b border-teal-300/70" />
        <div className="absolute bottom-3 left-1/2 h-10 w-24 -translate-x-1/2 rounded-t-full border-x border-t border-teal-300/70" />
        <div
          className="absolute h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-teal-600 shadow-[0_0_20px_rgba(13,148,136,0.35)]"
...
export function PlayerProfileCard({ categories }) {
  ...
  return (
    <ReportCard className="min-h-[360px]">
      ...
      <h3 className="mt-2 text-xl font-black text-slate-950">Performance dimensions</h3>
          ...
          return (
            <div key={category.key}>
              <div className="mb-1 flex items-center justify-between text-xs">
                <span className="font-black uppercase tracking-[0.12em] text-slate-800">{category.label}</span>
                <span className="font-black tabular-nums" style={{ color: tone.color }}>{score !== null ? Math.round(score) : "-"}</span>
              </div>
              <div className="h-2.5 overflow-hidden rounded-full border border-slate-200" style={{ backgroundColor: tone.track }}>
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="rounded-full border border-slate-200 px-2 py-0.5 text-xs font-black text-slate-900">{count}</span>
      </div>
      <div className="space-y-2">
        {rows.length ? rows.slice(0, 5).map((row) => {
          const tone = percentileColor(row.percentile);
          return (
            <div key={`${type}-${row.key}`} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
              ...
              <span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.1em] ${tone.border} ${tone.bg} ${tone.text}`}>{row.level}</span>
            </div>
          );
...
export function CharacteristicsCard({ characteristics }) {
  return (
    <ReportCard className="min-h-[360px]">
      ...
      <h3 className="mt-2 text-xl font-black text-slate-950">Strengths and weaknesses</h3>
  ...
  return (
    <ReportCard className="min-h-[310px]">
      <div className="mb-4 flex items-center justify-between">
        <h4 className="text-sm font-black uppercase tracking-[0.2em] text-slate-950">{group.label}</h4>
          ...
          return (
            <div key={metric.key}>
              <div className="mb-1 flex items-center justify-between gap-3 text-xs">
                <span className="min-w-0 truncate font-bold text-slate-800">{metric.label}</span>
                <span className="shrink-0 font-black tabular-nums" style={{ color: rawMode ? "#0f172a" : tone.color }}>{rawMode ? formatValue(metric.raw, metric.format) : formatValue(percentile, "integer")}</span>
              </div>
              <div className="h-2.5 overflow-hidden rounded-full border border-slate-200" style={{ backgroundColor: tone.track }}>
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button type="button" onClick={() => setRawMode(!rawMode)} className={`rounded-full border px-4 py-2 text-xs font-black uppercase tracking-[0.14em] ${rawMode ? "border-teal-600 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-600"}`}>Raw values</button>
        </div>
      </div>
      ...
      <div className="mt-4 h-[360px]">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={data} outerRadius="82%">
            ...
            <PolarRadiusAxis domain={[0, 100]} ticks={[0, 25, 50, 75, 100]} tick={{ fill: "#64748b", fontSize: 9 }} axisLine={false} tickLine={false} />
            ...
            <Radar name={player.name || "Player"} dataKey="player" stroke="#0f766e" fill="rgba(15,118,110,0.18)" fillOpacity={0.55} strokeWidth={2.5} />
          </RadarChart>
        </ResponsiveContainer>
      </div>
    </ReportCard>
    ...
    <ReportCard className="min-h-[420px]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-xl font-black text-slate-950">Metric profile by season</h3>
          <p className="mt-1 text-xs font-semibold text-slate-500">Same radar metrics as the comparison chart, using {context} percentiles.</p>
        </div>
        <div className="flex max-w-sm flex-wrap justify-end gap-2">
          {seasons.map((season, index) => (
            <span key={season.player_season_id || `${season.calendar}-${index}`} className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 px-2 py-1 text-[10px] font-black uppercase tracking-[0.1em] text-slate-600">
  ...
  return (
    <ReportCard className="min-h-[460px]">
      ...
      <div className="relative mt-4">
        ...
        />
        {showResults ? (
          <div className="absolute z-40 mt-2 max-h-72 w-full overflow-auto rounded-2xl border border-slate-300 bg-white shadow-xl">
            {results.length ? results.map((item) => (
              <button key={`${item.id}-${item.player_season_id}`} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => onSelect(item)} className="block w-full px-4 py-3 text-left text-sm text-slate-700 hover:bg-teal-50">
      ...
      <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200">
        <div className="grid grid-cols-[1.15fr_0.8fr_0.8fr] bg-slate-50 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-teal-700">
        ...
        {keys.map((key) => {
          ...
          return (
            <div key={key} className="grid grid-cols-[1.15fr_0.8fr_0.8fr] border-t border-slate-200 px-3 py-2 text-sm">
              <span className="min-w-0 truncate font-semibold text-slate-800">{config.label}</span>
              <span className={`text-right font-black tabular-nums ${aBetter ? "text-teal-700" : bBetter ? "text-red-600" : "text-slate-900"}`}>{formatValue(a, config.format)}</span>
              <span className={`text-right font-black tabular-nums ${bBetter ? "text-teal-700" : aBetter ? "text-red-600" : "text-slate-900"}`}>{formatValue(b, config.format)}</span>
            </div>
          );
        })}
      </div>
    ...
    <ReportCard>
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-xl font-black text-slate-950">Top 10 statistical neighbors</h3>
          <p className="mt-1 text-xs font-semibold text-slate-500">Sorted by Next Legend global score, descending.</p>
        </div>
        <span className="rounded-full border border-slate-200 px-3 py-1 text-xs font-black text-teal-700">{sortedSimilarities.length}</span>
      </div>
      ...
      {sortedSimilarities.length ? (
        <div className="space-y-2">
          <div className="hidden grid-cols-[52px_minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,1fr)_90px_90px] gap-3 px-4 text-[10px] font-black uppercase tracking-[0.14em] text-slate-400 md:grid">
            <span>#</span>
            <span>Player</span>
            <span>Club</span>
            <span>Context</span>
          ...
          {sortedSimilarities.map((sim, index) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => onOpen(sim)}
                className="grid w-full gap-3 rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-teal-600 hover:shadow-md md:grid-cols-[52px_minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,1fr)_90px_90px] md:items-center"
              >
                <div className="flex h-9 w-9 items-center justify-center rounded-md bg-teal-700 text-sm font-black text-white">
                  {index + 1}
                </div>
                <div className="flex min-w-0 items-center gap-3">
                  {photoUrl ? (
                    <img src={photoUrl} alt={sim.player_b_name || "Player"} className="h-12 w-12 shrink-0 rounded-full border border-slate-200 object-cover" loading="lazy" />
                  ) : (
                    <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-100 text-sm font-black text-slate-600">
                      {getInitials(sim.player_b_name)}
                    </span>
                  )}
                  <div className="min-w-0">
                    <p className="truncate text-sm font-black text-slate-950">{sim.player_b_name || "-"}</p>
                ...
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="truncate text-sm font-bold text-slate-800">{sim.team || "-"}</span>
                </div>
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-slate-800">{sim.competition_name || "-"}</p>
                  <p className="mt-1 truncate text-xs text-slate-500">{sim.calendar || "-"}</p>
                </div>
                <div className="rounded-md bg-slate-50 p-2 md:bg-transparent md:p-0 md:text-right">
                  <p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 md:hidden">Age</p>
                  <p className="font-black text-slate-950">{formatValue(sim.age, "integer")}</p>
                </div>
                <div className="rounded-md bg-teal-50 p-2 md:bg-transparent md:p-0 md:text-right">
                  <p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 md:hidden">Score</p>
                  <p className="text-lg font-black text-teal-700">{formatValue(sim.global_score_adjusted, "score")}</p>
                </div>
              </button>
            );
          })}
