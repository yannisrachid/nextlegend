...
export default function YouthRankingPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("");
      try {
        const res = await fetchJson("/youth/ranking/page", {
          ...filters,
        ...
        if (!cancelled) setError(err.message || "Unable to load Youth ranking.");
      } finally {
  ...
  const seasons = meta?.seasons || [2026];
      ...
      <div className="mx-auto max-w-[1500px] space-y-6">
        <header className="nl-page-header">
          ...
          <div className="mt-2 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl font-semibold tracking-normal text-white md:text-4xl">Eyeball youth ranking</h1>
              <p className="mt-2 max-w-3xl text-sm text-[#A0A8A3]">
                Age-adjusted cohorts with position-specific scoring, birth-year context and competition-level percentiles.
              </p>
            </div>
            <div className="grid grid-cols-3 gap-2 text-right">
              <div className="rounded-md border border-white/10 bg-white/[0.035] px-3 py-2">
                <p className="text-lg font-semibold text-white">{formatNumber(meta?.counts?.rows || 0)}</p>
                <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#6F7772]">Rows</p>
              </div>
              <div className="rounded-md border border-white/10 bg-white/[0.035] px-3 py-2">
                <p className="text-lg font-semibold text-white">{formatNumber(meta?.counts?.players || 0)}</p>
                <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#6F7772]">Players</p>
              </div>
              <div className="rounded-md border border-[#3A8967]/30 bg-[#2F7D5C]/15 px-3 py-2">
                <p className="text-lg font-semibold text-[#8CC7A7]">{formatNumber(total)}</p>
                <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#8CC7A7]">Cohort</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <Card className="nl-filter-bar p-0">
          <div className="flex flex-col gap-4 border-b border-white/10 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-lg font-semibold text-white">Filter the youth universe</h2>
            </div>
          ...
          <div className="space-y-4 p-4">
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-[120px_minmax(260px,1.2fr)_minmax(220px,1fr)_minmax(150px,0.7fr)]">
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-season">Season</Label>
                <Select id="youth-season" value={filters.season} onChange={(event) => updateFilter({ season: Number(event.target.value) })}>
                  {seasons.map((season) => <option key={season} value={season}>{formatYouthSeason(season)}</option>)}
                </Select>
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-championship">Competition / level</Label>
                <Select id="youth-championship" value={filters.championship} onChange={(event) => updateFilter({ championship: event.target.value })}>
                  {championships.map((item) => <option key={item} value={item}>{item}</option>)}
                </Select>
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-club">Club</Label>
                <input
                  id="youth-club"
                  className="nl-field"
                  value={filters.club}
              ...
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-age-category">Age category</Label>
                <Select id="youth-age-category" value={filters.age_category} onChange={(event) => updateFilter({ age_category: event.target.value })}>
                  {ageCategories.map((item) => <option key={item} value={item}>{item}</option>)}
                </Select>
              </div>
            </div>

            <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(220px,1fr)_minmax(160px,0.7fr)_minmax(220px,1fr)_minmax(140px,0.65fr)_minmax(140px,0.65fr)]">
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-position-group">Position group</Label>
                <Select id="youth-position-group" value={filters.position_group} onChange={(event) => updateFilter({ position_group: event.target.value })}>
                  {positionGroups.map((item) => <option key={item} value={item}>{item}</option>)}
                </Select>
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-position">Position</Label>
                <Select id="youth-position" value={filters.position} onChange={(event) => updateFilter({ position: event.target.value })}>
                  {positions.map((item) => <option key={item} value={item}>{item}</option>)}
                </Select>
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-birth-year">Birth year</Label>
                <Select id="youth-birth-year" value={filters.birth_year} onChange={(event) => updateFilter({ birth_year: event.target.value })}>
                  <option value="">All birth years</option>
                  {birthYears.map((item) => <option key={item} value={item}>{item}</option>)}
                </Select>
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="youth-min-minutes">Min minutes</Label>
                <input
                  id="youth-min-minutes"
                  type="number"
                  inputMode="numeric"
                  min={0}
                  step={1}
        ...
        <Card className="overflow-hidden p-0">
          <div className="flex flex-col gap-3 border-b border-white/10 px-4 py-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs font-semibold text-[#6F7772]">
                Page {page + 1} / {totalPages} - {formatNumber(total)} rows
              </p>
            </div>
          ...
          {error ? (
            ...
            <div className="space-y-2 p-4">
              {Array.from({ length: 8 }).map((_, index) => <div key={index} className="nl-skeleton h-14 rounded-md" />)}
            </div>
          ) : items.length === 0 ? (
            <div className="p-6">
              <div className="nl-empty-state">
                <p className="text-sm font-semibold text-white">No youth players match these filters.</p>
                <p className="mt-1 text-xs text-[#6F7772]">Broaden the cohort or lower the minutes threshold.</p>
              </div>
            </div>
          ) : (
                    ...
                    <tr key={row.id} className="align-middle">
                      <td className="px-4 py-3">
                        <span className="flex h-8 w-8 items-center justify-center rounded-md border border-[#3A8967]/30 bg-[#2F7D5C]/15 text-xs font-semibold text-[#8CC7A7]">
                          {page * filters.limit + index + 1}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <Link href={`/youth-scouting/reports?youth_id=${row.id}`} target="_blank" rel="noreferrer" className="flex min-w-0 items-center gap-3">
                          <span className="flex h-10 w-10 items-center justify-center rounded-md border border-white/10 bg-white/[0.04] text-xs font-semibold text-[#8CC7A7]">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex max-w-[320px] flex-wrap gap-1.5">
                          {[row.position_group, row.position, row.age_category, `${formatNumber(row.minutes_played)} mins`].filter(Boolean).map((item) => (
                      ...
                      <td className="px-4 py-3 text-right text-white">{formatScore(row.rating)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex flex-col gap-3 border-t border-white/10 px-4 py-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="youth-limit">Rows per page</Label>
              <div className="w-24">
                <Select id="youth-limit" value={filters.limit} onChange={(event) => updateFilter({ limit: Number(event.target.value) })}>
